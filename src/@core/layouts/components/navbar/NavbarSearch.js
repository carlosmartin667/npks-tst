    ...
    return ( <
        ...
        /div> {
            navbarSearch ? ( <
                ...
                grouped = { true }
                placeholder = 'Explore Npks...'
                autoFocus = { true }
                onSuggestionItemClick = { handleSuggestionItemClick }
                externalClick = { handleExternalClick }
                clearInput = {
                    (userInput, setUserInput) => handleClearInput(setUserInput) }
                onKeyDown = { onKeyDown }
                onChange = { e => dispatch(handleSearchQuery(e.target.value)) }
                customRender = {
                    (item, i, filteredData, activeSuggestion, onSuggestionItemClick, onSuggestionItemHover) => {
                        ...
                        return ( <
                            ...
                            div className = 'item-container d-flex' > {
                                item.icon ? ( <
                                    ...
                                    img className = 'rounded-circle mt-25'
                                    src = { item.img }
                                    height = '28'
                                    width = '28'
                                    alt = { item.title }
